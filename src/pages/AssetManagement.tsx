...
const AssetManagement = () => (
  ...
  <div className="min-h-screen">
    <SEO title="Asset Management" description="Institutional-grade investment strategies across equities, fixed income, alternatives and private markets — CF Banque Investment." path="/asset-management" />
    <Navbar />
    <PageHero
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
            ...
            <div className="space-y-3">
              {clientTypes.map((client) => (
                <div key={client} className="flex items-center gap-3 bg-muted p-4 border">
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {objectives.map((obj, i) => (
            <motion.div key={obj.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="bg-card p-8 border text-center">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-16">
            {["Global equity portfolios", "Fixed income investment strategies", "Balanced multi-asset funds", "Institutional discretionary mandates", "Thematic investment vehicles"].map((item) => (
              <div key={item} className="flex items-start gap-3 bg-muted border p-4">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="mt-16">
          ...
          <div className="space-y-6">
            {portfolioProcess.map((step, i) => (
              <div key={step.title} className="flex gap-6 items-start bg-card border p-6">
            ...
            <div>
              ...
              <div className="space-y-4">
                {creditAnalysis.map((item) => (
                  <div key={item.title} className="bg-card border p-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            ...
            <div>
              <div className="bg-primary text-white p-8">
                <Leaf className="w-10 h-10 text-accent mb-6" strokeWidth={1.5} />
